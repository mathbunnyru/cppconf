...
public:
    ...
    void swap(expected& other)
    {
        ...
        if (m_valid) {
            if (other.m_valid) {
                ...
                std::swap(m_valid, other.m_valid);          // swap the isValid flags
            }
    ...
    T& get()
    {
        if (!m_valid) THROW_IF_EXCEPTIONS_ARE_ENABLED("expected<T, E> contains no value");
        return m_value;
    }
    ...
    E& error()
    {
        if (m_valid) THROW_IF_EXCEPTIONS_ARE_ENABLED("There is no error in this expected<T, E>");
        return m_error;
    }
    ...
    void swap(expected& other)
    {
        ...
        if (m_valid) {
            if (other.m_valid) {
                ...
                std::swap(m_valid, other.m_valid);       // swapping the isValid flags
            }
...
template <typename T,
          ...
          typename Function,
          typename ResultType =
              decltype(std::declval<Function>()(std::declval<T>()))>
